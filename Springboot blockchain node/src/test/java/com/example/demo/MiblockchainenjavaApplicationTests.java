...
public class MiblockchainenjavaApplicationTests {
	...
	public void añadirTransaccion() throws Exception {
		...
		tx.setFirma(UtilidadesFirma.firmar(tx.getContenidoFirma(), claveEmisor.getPrivate().getEncoded()));
		tx.setHash(tx.calcularHashTransaccion());
		
		 System.out.println(asJsonString(tx));
	...
	public void getIpPublica() throws Exception {
		mvc.perform(MockMvcRequestBuilders.get("/nodo/ip").contentType(MediaType.APPLICATION_JSON)); //corregido;)
	}

	@Test
	public void altaNodo() throws Exception {
		mvc.perform(MockMvcRequestBuilders.post("/nodo").content("http:\\localhost:8090")
	...
	public void bajaNodo() throws Exception {
		...
		mvc.perform(MockMvcRequestBuilders.delete("/nodo").content("http:\\localhost:8090")
				.contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
	}
